...
test.group('SmsFly', () => {
  ...
  test('send sms to multi recipients', async (assert) => {
    const config = {
      username: process.env.SMSFLY_USERNAME,
      password: process.env.SMSFLY_PASSWORD
    }
    ...
    const res = driver._generatePayload({
      to: [process.env.TEST_PHONE, process.env.TEST_PHONE],
      text: 'Hello world'
    })

    assert.equal(res, '<?xml version="1.0" encoding="utf-8"?><request><operation>SENDSMS</operation><message start_time="AUTO" end_time="AUTO" lifetime="4" desc="description"><recipient>+380956353711</recipient><recipient>+380956353711</recipient><body>Hello world</body></message></request>')
  }) */
})
